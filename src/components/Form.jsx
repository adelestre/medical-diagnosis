...
function Form({ symptoms, addSymptom, removeSymptom }) {
  const [filteredSymptoms, setFilteredSymptoms] = useState(db.faits)
  const [filterOpen, setFilterOpen] = useState(false)
  const [input, setInput] = useState('')
  const onInputChange = (e) => {
    setInput(e.target.value)
    setFilteredSymptoms(
      db.faits.filter((item) => {
        return (
  ...
  return (
    <div className="relative p-6 pr-4 pt-2 rounded-md h-full bg-gray-200">
      <form className="relative flex flex-col pr-2 mb-2">
        {false && (
          <div
            className="flex flex-row w-min items-center cursor-pointer"
      ...
      <ol className="relative w-full h-96 flex flex-col gap-1 pr-2 overflow-y-auto">
